...
for epoch in range(maxEpoch):
    ...
    for batch in batchListTrain:
        ...
        # train = True is not valid for processBatchInfoNMT. It is used in processBatchInfoVocGen function.
        #  
        ...
        targetVocGen, inputVocGen = corpus.processBatchInfoVocGen(batchData, train = True, device = device)
        ...
        if batchProcessed == len(batchListTrain)//2 or batchProcessed == len(batchListTrain):
            ...
            for batch in batchListDev:
                batchSize = batch[1]-batch[0]+1
                batchInputSource, lengthsSource, batchInputTarget, batchTarget, lengthsTarget, tokenCount, batchData, maxTargetLen = corpus.processBatchInfoNMT(batch, device = device) # train = False removed

                targetVocGen, inputVocGen = corpus.processBatchInfoVocGen(batchData, smoothing = False, train=False device = device) # train = False added.
                outputVocGen = vocGen(inputVocGen)
