...
@register_model('fconv_dvoc')
class FConvDvocModel(FairseqEncoderDecoderModel):
    """
    A fully convolutional model (extended with dynamic vocabularies), i.e. a convolutional encoder and a
    ...
    def build_model(cls, args, task):
        ...
        decoder = FConvDecoder(
            ...
            enable_dvoc=args.enable_dvoc,
            dvoc_k=args.dvoc_K,
        )
        return FConvDvocModel(encoder, decoder)
    
    def forward(self, src_tokens, src_lengths, prev_output_tokens, dynamic_vocab, **kwargs):
        encoder_out = self.encoder(src_tokens, src_lengths=src_lengths, **kwargs)
        decoder_out, avg_attn_scores = self.decoder(prev_output_tokens, encoder_out=encoder_out, dynamic_vocab=dynamic_vocab, **kwargs)
...
class SmallSoftmax(nn.Module):
    ...
    def forward(self, input, output_list = None, full_softmax = False):
        if output_list is not None:
            ...
            padded_output=torch.cat((torch.zeros(output_list.size(0),4).long().to(output_list.device), output_list), 1)
            self.setSubset(padded_output)
...
class FConvEncoder(FairseqEncoder):
    ...
    def forward_non_torchscript(self, net_input: Dict[str, Tensor]):
        encoder_input = {
            ...
            if k != "prev_output_tokens" and k != "dynamic_vocab"
        }
...
class FConvDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(
        ...
        adaptive_softmax_cutoff=None, adaptive_softmax_dropout=0, enable_dvoc=False, dvoc_k=1000
    ):
        super().__init__(dictionary)
    ...
    def forward(self, prev_output_tokens, encoder_out=None, incremental_state=None, dynamic_vocab=None, **unused):
...
@register_model_architecture('fconv_dvoc', 'fconv_dvoc')
